...
## Validation behavior for missing bounds

For `PUT /time-unit`, `up_bnd` and `low_bnd` are validated against existing Time Bound records before write operations. If one or both referenced bounds are missing, the request is rejected with `403` and a structured validation array instead of returning `500`.
...
This contract is intentionally consumable by frontend validation/error-notification logic.
