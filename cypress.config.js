...
async function waitForDatabase(options = {}, env = {}) {
  ...
  const timeoutMs = Number(options.timeoutMs ?? env.dbWaitTimeoutMs ?? DEFAULT_DB_WAIT_TIMEOUT_MS)
    ...
    const attemptConnection = () => {
      ...
      const scheduleRetry = error => {
        ...
        if (Date.now() >= deadline) {
          reject(new Error(`Timed out waiting for MariaDB at ${host}:${port} (${error.message})`))
          return
        }
