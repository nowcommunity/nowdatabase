...
const normalizedCountryLookup = new Map<string, string>(
  validCountries.map(country => [country.toLowerCase(), country]),
);
...
export const matchesCountryOrContinent = (
  ...
  return continentAliases[continent].some(
    alias => alias.includes(normalizedFilter) || normalizedFilter.includes(alias),
  );
};
