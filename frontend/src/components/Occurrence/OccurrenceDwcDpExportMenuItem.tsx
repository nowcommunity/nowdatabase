...
export const OccurrenceDwcDpExportMenuItem = ({ handleClose }: { handleClose: () => void }) => {
  ...
  const fetchZipFile = async () => {
    setLoading(true)
    notify('Generating DwC-DP ZIP export, please wait...', 'info', null)

    try {
      const response = await fetch(`${BACKEND_URL}/occurrence/export/dwc-data-package`, fetchOptions)
      if (!response.ok) {
      ...
      const showDownloadProgress = () => {
        if (!closed) {
          setTimeout(() => {
            setNotificationMessage(`Downloading DwC-DP ZIP, ${Math.round((bytes / 1000000) * 10) / 10} MB`)
            showDownloadProgress()
          }, 500)
        }
      }

      notify('Downloading DwC-DP ZIP...', 'info', null)
      showDownloadProgress()
      ...
      notify('Downloading DwC-DP export failed.', 'error')
    } finally {
      setLoading(false)
    }
  }
