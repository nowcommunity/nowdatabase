...
export const LocalityDwcExportMenuItem = ({ handleClose }: { handleClose: () => void }) => {
  ...
  const filename = `now_dwc_localities_test_export_${currentDateAsString()}.zip`
  ...
  const fetchZipFile = async () => {
    ...
    try {
      await downloadExportFileWithProgress({
        url: `${BACKEND_URL}/locality/export/dwc-archive`,
        filename,
        fetchOptions,
        notify,
        setNotificationMessage,
        startMessage: 'Generating DwC-A locality ZIP export...',
        waitingMessage: 'Waiting for locality export rows',
        downloadMessage: 'Downloading DwC-A locality ZIP',
