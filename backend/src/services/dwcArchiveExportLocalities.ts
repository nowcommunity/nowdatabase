...
export const buildDwcLocalityArchiveZipBuffer = async (localityIds?: number[]): Promise<Buffer> => {
  if (localityIds && localityIds.length === 0) return await buildDwcLocalityArchiveZipBufferFromLocalities([])
